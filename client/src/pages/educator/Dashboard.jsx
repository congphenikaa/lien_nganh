...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const { data} = await axios.get(backendUrl + '/api/education/dashboard',{headers: {Authorization: `Bearer ${token}`}})
      if(data.success){
    ...
    md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <div className='space-y-5'>
        <div className='flex flex-wrap gap-5 items-center'>
          ...
          <div>
            <h2 className='pb-4 text-lg font-medium'>Last Enrolments</h2>
                ...
                <tbody className='text-sm text-gray-500'>
                  {dashboardData.enrolledStudentsData.map((item, index) => (
                    <tr key={index} className='border-b border-gray-500/20'>
                      ...
                      <td className='md:px-4 px-2 px-3 flex items-center space-x-3'>
